 ...
 */
import { App, ErrorBoundary, HttpError } from "@bundar/core";
...
function roleOf(context: Context): AdminRole | null {
  const session = getSession(context);
  const role = session?.get("role") as AdminRole | undefined;
  return role !== undefined && role in ROLE_RANK ? role : null;
}

function userOf(context: Context): string | null {
  const session = getSession(context);
  return (session?.get("user") as string | undefined) ?? null;
}

/** Server-side gate: session role only — never HTMX metadata. */
function requireRole(context: Context, minimum: AdminRole): string | null {
...
function parseStatus(value: string | null): ArticleStatus | null {
...
export function createAdminApp(options: CreateAdminAppOptions): {
  ...
  app.group("", (routes) => {
    routes.use(csrfMiddleware({ secret: csrfSecret }));
    ...
    async (context) => {
      ...
      const content = [
        ...
        jsx("div", {
          id: "form-region",
          children:
            ROLE_RANK[role ?? "viewer"] >= ROLE_RANK.editor
              ? articleForm({
                  action: "/articles",
                  token: token.token,
                  title: "",
