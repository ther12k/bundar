...
## 2026-08-28 — GH-180: additive Input/Result executable form-action contract

- `@bundar/forms` gained `ExecutableFormActionDefinition<Input, Result>` (contracts.ts, exported from index.ts): the additive second contract separating VALIDATED INPUT from the DOMAIN RESULT. `run(input, context)` executes the business action exactly once inside the transaction hooks; `buildFragment(result, context)` renders from the domain result; `renderForm(render, context)` keeps the invalid path. Every lifecycle hook receives the neutral `FormWorkflowContext`. Delivery stays an opaque adapter-owned record — no HTMX concept enters the package.
- Legacy `FormActionDefinition<Output>` is untouched and source-compatible; type tests pin its distinctness (a legacy definition cannot satisfy the executable contract without `run()`). Executor and htmx facade changes are deliberately out of scope (GH-181/GH-182/GH-183 depend on this one).
- Type tests (`packages/forms/test/types/executable-form-action.test-d.ts` + wrapper, core's `Expect`/`Equal` pattern): exact key set (no wire concepts), Input/Result inference and separation, `@ts-expect-error` proofs that `run` takes validated input and success rendering takes the domain result. FORMS_CONTRACT_VERSION bumps 1.1.0 → 1.2.0 for the additive surface (runtime drift test key set unchanged — type-only addition).
- Focused gates on branch `feat/forms-input-result-contract` (fcb62cc): `bun test packages/forms` 19 pass / 0 fail; `bun run typecheck` exit 0; `bun run lint` exit 0 (initial unused-param errors in the test-d fixed by genuinely using every hook param at runtime); `bun run build` exit 0. Formatting fixup ef929ce.
- Candidate regeneration on merged main ef929ce: 9-step sequence (publish:dry-run, pack:audit, release:sbom, prettier, release:provenance, release:reproduce, release:notes-check, release:verify, registry:verify --preflight) all exit 0; manifest rebound to ef929ce; conformance/security/evidence artifacts reformatted to Prettier style.
- Full battery `ci:release` on the rebound tree: every functional gate green — full suite 1,234 tests / 0 fail (154 files), release verification 42 checks, e2e + scaffold matrix ALL CHECKS PASSED — except the FINAL `bench:regression` budget gate: `startup:bundar:readyMs 48.6–49.7 > fail 44.0` (alerts: raw-bun readyMs 6.2 > 6.0; validated-form ratio 5.83×/static 1.13×/dynamic 1.82×).
- Honest attribution: the startup drift is PRE-EXISTING machine drift, not this change. Decisive experiment: checked out ed228ae (main BEFORE this wave) and re-recorded — `startup:bundar:readyMs` 50.2ms, same failure; raw-bun baseline itself reads ~5.4ms p50 vs 3.86 budget p50 (≈1.6×) and carno 142ms, i.e. environment-wide slowdown since budgets were generated 2026-08-27. Bun version unchanged (1.4.0). A type-only + constant change cannot move process startup; the elevated raw-bun baseline proves the platform, not the framework. Budgets were NOT touched (no gate weakening inside an issue's write set); machine-drift budget regeneration is filed separately for maintainer decision.
- Mid-wave false alarm documented: publisher-safety/loader-wave tests fail while the source tree is dirty or the candidate manifest lags HEAD ("SOURCE CHANGED since manifest SHA") — by design (BR-111 identity guard); they passed once the tree was committed and the manifest rebound.
- Public battery, two runs (honest record): 33181030355 on 469d701 FAILED at format:check — the last local battery attempt had rewritten bench/conformance artifacts after the final format gate, and that unformatted state was committed; formatting-only fix 8aefdd0 (content identical). Rerun 33181227322 on 8aefdd0 SUCCESS: bundle `release-candidate-artifacts-8aefdd052618d3308b0b9339d1f74ba2bf59ac0b`, GitHub artifact digest sha256:2894d29f28570d3ec996b613bcd9d9c220f8765ce75c1038a7a3b0e85e2cae51. The runner's bench:regression passed its startup budgets, confirming the local failure is workstation-specific drift (#191).
- Model B publish-path rehearsal from the downloaded bundle: `publish:approved --dry-run --manifest … --tarball-root …` exit 0 (NOTHING published) and `registry:verify --preflight --manifest … --root-dir …` 9/9 on-disk SHA-256 matches. (First preflight attempt used `--tarball-root`, which registry:verify does not accept — it silently read the local repo tarballs and mismatched; correct flag is `--root-dir`. Operator error, recorded here.)
- Net wave result: GH-180 landed on main 8aefdd0; all functional gates green locally and in the public battery; the only red anywhere is the local workstation's stale bench budgets (#191, maintainer decision pending).

## 2026-08-28 — GH-181 + GH-182: parallel wave — executable form-action executor and invalidField helper

- Two isolated worktrees/branches implemented the unblocked children of the Forms API epic in parallel and merged in that order (executor first — it establishes runtime semantics), with the barrel wired in a small integration commit afterward; #183 remains untouched pending both merges.
- GH-181 (`feat/executable-form-executor`, 80bcc78): additive `executeExecutableFormAction<Input, Result>(context, definition, adapter)` in `packages/forms/src/run-executable-form-action.ts` runs the required exact sequence — cancellation checkpoint → bounded parse → validation → invalid path unchanged (422/"Validation failed"/render/formTarget) → transaction.begin → `run(input, context)` exactly once → `buildFragment(result, context)` → commit → adapter.valid. Rollback covers BOTH run() and fragment failures exactly once (rendering-before-commit preserved); the pre-begin checkpoint keeps an abort from stranding an open transaction, and a post-run checkpoint routes post-mutation aborts through rollback (no commit lie). Legacy `executeFormAction` stayed byte-for-byte untouched — the small cancellation helpers are deliberately duplicated until GH-184 proves equivalence.
- GH-181 tests: runtime suite proves begin→run→fragment→commit order, invalid-never-runs, exactly-once commit, exactly-once rollback on run/fragment failure AND on post-run abort, sync + async callbacks, abort before pipeline / after validation, exact RESULT identity (`buildFragment` receives the exact object `run` returned — `toBe`, not deep equality) and exact CONTEXT identity (`run` and `buildFragment` receive the same workflow context object, `toBe` the executor's argument). Type tests pin separate Input/Result inference through the executor and the legacy shape's rejection (`@ts-expect-error`).
- GH-182 (`feat/invalid-field-helper`, 8b645ee): framework-neutral `invalidField(render, name)` + `InvalidFieldView` in `packages/forms/src/invalid-field.ts` — `value` (first submission), `values` (all, in order), `multiple`, `errors` (field's own, original order), `error` (first field message only), `invalid`. Duplicates stay observable (never `String(array)`); globals never leak into a field; empty submission arrays render as missing; redaction stays owned upstream (sensitive keys are absent from `submitted` → missing). Barrel export + FORMS_CONTRACT_VERSION 1.2.0 → 1.3.0 + drift-list pin landed with the branch.
- Integration commit c0fe656: `executeExecutableFormAction` added to the barrel beside `invalidField`; drift list pins both; version stays 1.3.0 (one additive-surface bump covers the wave — disclosed).
- Gates: per-branch `bun test packages/forms` 31 and 30 pass / 0 fail; merged tree 42 pass / 0 fail across 7 forms files; full suite 1,257 pass / 0 fail across 158 files (after regeneration); typecheck, lint, format:check, build all exit 0. Candidate regeneration 9-step sequence exit 0 with manifest rebound to c0fe656; release:verify 42/42; registry preflight 9/9. Local `ci:release` intentionally not run to green this wave: its only failing step remains the documented workstation bench drift (#191) — budgets untouched; the public battery is the authority. (The three publisher/loader test failures seen mid-wave were the BR-111 identity guard rejecting the pre-regeneration manifest binding — expected, resolved by the rebind.)
- Authoritative battery 33185978234 on main 3c9461a: success; bundle `release-candidate-artifacts-3c9461afff69d990631dc115e3f69b6b68cd4073`, GitHub artifact digest sha256:8eed55d87aa73f409f28ca1390df66dd2e366cac5b748f78071c767e0d48939b. Model B rehearsal from the downloaded bundle green: `publish:approved --dry-run --manifest/--tarball-root` exit 0 (NOTHING published) and `registry:verify --preflight --manifest/--root-dir` 9/9 on-disk SHA-256 matches.

## 2026-08-28 — GH-181 residual + GH-183: post-fragment abort checkpoint and the htmx form-action facade

- Executor residual first (105c1ae): `executeExecutableFormAction` gained the missing cancellation checkpoint between asynchronous fragment resolution and `transaction.commit` — an abort firing while a fragment builder was pending could previously resolve and still commit. The checkpoint sits inside the existing try, so post-rendering aborts roll back exactly once instead of committing a response nobody will observe. Regression "abort during async fragment rendering rolls back" proven to FAIL without the checkpoint and pass with it.
- GH-183 (feat/htmx-form-action-facade, ad958d4): additive `@bundar/htmx` facade over the separated workflow — `HtmxFormActionDefinition<Input, Result>` (schema → run → result → success/invalid presentation split), `InvalidFormView` extending `InvalidFormRender` with `field(name)` built on GH-182's `invalidField` via structural spread (no mutation), zero-runtime `defineFormAction()` for inference, and `createFormActions({ dialect })` exposing `execute()` (discriminated outcome) and `handle()` (direct Response) with the execute closure captured (no `this`).
- Pure mapping, no delivery duplication: schema/validation/run pass through to `executeExecutableFormAction`; `success.fragment` becomes `buildFragment`; the delivery record carries only the ActionOptions fields; `errorViewResponse()` still owns document/fragment negotiation, retarget (server-known `invalid.target` → HX-Retarget) and reswap (outerHTML pairing); `actionResponse()` still owns PRG vs enhanced composition, status validation, directives, cache policy and Vary. No raw HX-* header logic was added. The generic BR-088 fallback document moved verbatim to `error-document.ts` so both adapters share one fallback (small private extraction, zero behavior change to the legacy path).
- Type proofs: the createTodo inference chain (Input from schema, Result from run flowing exactly into success.fragment via `Expect<Equal>`), `@ts-expect-error` rejections for fragment-as-Input, run-as-Result, non-approved status/redirectStatus values, and missing invalid presentation; facade method shapes pinned (`execute` → `Promise<FormActionOutcome>`, `handle` → `(context) => Promise<Response>`); definition key surface pinned exactly (keyof materialized through a homomorphic mapped type — the generic alias keeps keyof deferred for the strict Equal trick).
- Runtime matrix green: valid ordinary (run+fragment once, PRG 303, Location preserved, empty body); valid enhanced (fragment bytes, configured 201, HX-Trigger directive, Vary, no-store, no redirect); invalid enhanced (run never, field("title") retained value + field error exposed, 422, HX-Retarget #todo-card, HX-Reswap outerHTML); invalid ordinary with and without application document (document renderer receives 422 view + context; generic fallback used otherwise — no dangling field anchor links, region fragment never doubles as document); facade behavior (dialect decode path used, handle returns Response directly, outcome preserved, definition object never mutated, same context object reaches run/fragment/invalid renderer, transaction hooks bracket the valid path); legacy isolation (runFormAction untouched and passing, legacy FormActionDefinition unchanged, no export removed).
- Merge order per plan: residual → facade. Gates: per-branch htmx+forms 271 pass / 0 fail; merged full suite 1,279 pass / 0 fail across 160 files; typecheck, lint, build, format:check exit 0; candidate regeneration 9-step exit 0 with manifest rebound to 6415b58; release:verify 42/42; registry preflight 9/9. Local ci:release still stops only at the documented workstation bench drift (#191) — budgets untouched; public battery is the authority. #184 (equivalence proof) is next; #185/#186 follow it.
- Authoritative battery 33190239992 on main b0796b0: success; bundle `release-candidate-artifacts-b0796b00254b4e1f7193489c64329ea0b08e88c4`, GitHub artifact digest sha256:c2ccd07bb15f078ac5496894f7770bb2a9b3b4d17bd7ba9ae4ebbb95e327b0da. Model B rehearsal from the downloaded bundle green: `publish:approved --dry-run` exit 0 (NOTHING published) and `registry:verify --preflight --root-dir` 9/9 on-disk SHA-256 matches.

## 2026-08-28 — GH-184: differential legacy/facade conformance and the legacy cancellation alignment

- One shared scenario fixture (same body, schema, business mutation, result renderer, invalid renderers, transaction, dialect, delivery options) executes every scenario through BOTH `runFormAction()` and `defineFormAction()`/`createFormActions().execute()` — fresh scenario state and fresh request per surface, never a reused Request body — and compares COMPLETE observations: outcome kind, status/statusText, the canonical full header set (sorted), exact body bytes, thrown error name/message, transaction event sequence, mutation count, retained safe values, first error field. Not two independently-copied test suites: a single definition, two executions, one `toEqual`.
- Matrix green: ordinary valid (303 PRG, Location preserved, empty body, no Vary/cache headers on the redirect); enhanced valid (201, directive applied, privateContent, Vary, no-store, no Location); ordinary invalid (422 application-document bytes, region renderer never invoked, no enhanced headers); enhanced invalid (422 region bytes, HX-Retarget, HX-Reswap outerHTML); repeated/redacted retained values (title kept as ["x","second"], note scalar, password dropped; facade field("title") value/values/multiple aligned, no String(array) anywhere); ordered field/global errors (firstErrorField=title, issue order in bytes, global isolated); transaction success ([begin,mutate,fragment,commit]); mutation-failure and fragment-failure rollbacks (identical error name/message, exactly-once rollback); four cancellation scenarios (pre-aborted, abort during async validation, abort during success execution, abort during async fragment rendering) — all pairs byte- and event-identical.
- The paired suite exposed exactly the predicted genuine drift, and ONLY that: the legacy executor had no pre-begin cancellation checkpoint and no post-fragment checkpoint, so an abort firing during async fragment rendering left legacy COMMITTING the transaction while the facade rolled back. Aligned the legacy executor (`run-form-action.ts`): checkpoint moved before `transaction.begin()`, checkpoint added after fragment resolution INSIDE the rollback try. Regression proven load-bearing: reverted alignment → pair red (legacy "valid"/commit vs facade "error"/rollback); aligned → 12/12 green. Disclosed as a SAFETY ALIGNMENT: no public type changed, no contract version bump, facade checkpoints untouched.
- Honest fixture debugging recorded: two initial reds were fixture bugs, not engine drift — (1) runPaired originally shared one state across surfaces (events accumulated); (2) the abort scenarios initially never wired the controller's signal into the request context, so NEITHER surface aborted and both committed identically. Both fixed before judging engine equivalence; the genuine drift was then isolated and proven.
- Type fixture: both surfaces compile side by side; legacy `FormActionDefinition` inference unchanged; facade `Result` inference pinned (`Expect<Equal>`); `field(name)` and context types pinned. No public export removed; no drift-list change needed (test-only + one internal checkpoint hunk).
- Examples, starters, templates, create-bundar, and admin CRUD stay on the legacy API (merge gate: all six example variants pass). No executor consolidation — the duplicated helpers remain until a follow-up with this suite as safety net.
- Gates: forms+htmx 284 pass / 0 fail; full suite 1,292 pass / 0 fail across 162 files; typecheck, lint, build, format:check exit 0; test:example all six variants OK; candidate regeneration 9-step exit 0 (manifest rebound to 390b7f1); release:verify 42/42; registry preflight 9/9. Local ci:release still stops only at the documented workstation bench drift (#191); budgets untouched.
- Authoritative battery 33222608245 on main dbf8c12: success; bundle `release-candidate-artifacts-dbf8c12f…`, GitHub artifact digest sha256:6ebbef0b3c0249029246592b8986e7ae59b581c8f5fc596409f8f4c386456661. Model B rehearsal from the downloaded bundle green: `publish:approved --dry-run` exit 0 (NOTHING published) and `registry:verify --preflight --root-dir` 9/9 on-disk SHA-256 matches.

## 2026-08-28 — GH-185 + GH-186: adoption wave — Todo migration and starter generation on the separated facade

- Parallel wave, merged in the recommended order (#185 first — canonical full-feature reference; #186 rebased conceptually onto it, no file overlap so no conflicts; write sets strictly separated; one integration commit for artifacts/evidence/tracker per the collision rule).
- GH-185 (feat/todo-form-action-facade, bd54028): Todo create and edit migrated to `createFormActions({ dialect })` bound once in registerTodoRoutes + `defineFormAction`. `run()` owns repository mutation, flash, and post-mutation reads — create returns `{item, counts}` so `success.fragment` is genuinely pure (no repository reads while rendering); edit returns a discriminated `RenameTodoResult` (`renamed` | `not-found`) preserving the exact not-found behavior. Invalid fragment/document read `field("title").value/.error` — no `errors.first`, no casts, no `String(array)`; `forms.handle(definition)` returns the composed Response directly. Toggle/delete unchanged (not validated forms); CSRF, flash, OOB counts, typed URLs, PRG untouched.
- GH-185 proofs (facade.test.ts, 7 tests): mutation exactly once on valid create/edit, zero on invalid; pure success rendering; enhanced create carries item + OOB counts; invalid ordinary = full application document with retained title, aria-invalid/aria-describedby, fresh token; invalid enhanced = form fragment + HX-Retarget + outerHTML; duplicate titles → `field("title").value` is FIRST, never comma-joined (raw urlencoded body — record literals cannot carry duplicate keys); not-found unchanged. All 6 example variants, a11y, no-js suites green.
- GH-186 (feat/starter-form-action-facade, 5d316e0): templates/minimal subscribe migrated to the same shared pattern (facade bound once, run → Result, field(name) invalid rendering; view takes `InvalidFieldView` — no EMPTY_RENDER cast scaffolding). create-bundar minimal template now EMITS the facade pattern with an inline typed StandardSchema v1 — dependencies and package boundaries unchanged. facade-emission.test.ts pins the emitted source: createFormActions/defineFormAction binding, run/success/invalid/forms.handle shape, and absence of runFormAction/action.fragment-mutation/String(render.submitted)/errors.first/parseForm. test:template, test:scaffold (htmx2 default + htmx4 experimental ALL CHECKS PASSED), and the 19-step dx-cleanroom journey green.
- Disclosed drift lesson from the cleanroom journey: the neutral invalid envelope carries the generic "Validation failed" message — the SPECIFIC validation text now travels in field data. The emitted document renderer reads `field("email").error` (falling back to the envelope message). Legacy hand-rolled flow had inlined the specific message into the view; the facade makes the split explicit. Also honest debugging note: two initial fixture bugs (duplicate JS object keys collapse — raw body needed; one-time CSRF tokens — fresh token per POST) were fixed in the tests, not the engine; and the cleanroom "hang" was the tool keeping a failed journey's server alive behind a `tail` pipe — the real signal was the validation-message assertion.
- Integration tree d740e54: full suite 1,303 pass / 0 fail across 164 files; typecheck, lint, build, format:check exit 0; candidate regeneration 9-step exit 0 (manifest rebound to d740e54); release:verify 42/42; registry preflight 9/9. #191 and budgets untouched. #187 (docs, fed by the now-proven Todo + starter code) is the last child; #188 closes after it.
- Authoritative battery (adoption wave): first two runs failed on the runner and caught real portability issues — 33230038952: htmx:source-diff correctly flagged raw HX-* strings in the new example test (fixed via buildHtmxRequestHeaders neutral builder + dropping raw-header asserts, which the GH-184 suite already pins); 33230294533: template typecheck on the runner flagged SubscribedFragment/SubscribeForm used as JSX elements (TS2786, stricter runner tsc) — fixed to the template's function-call composition convention. Third run 33230594445 on main 5fe1a17 SUCCESS: bundle `release-candidate-artifacts-5fe1a17…`, digest sha256:749705d0d6b51f85843809e9f6276738708aeec622e6c7b54df777a1788dfeb1. Model B rehearsal from the downloaded bundle green (dry-run exit 0, NOTHING published; registry preflight 9/9).

## 2026-08-29 — GH-187: documentation wave — the facade as the documented preferred API

- Docs wave on docs/form-action-facade (af682e8), merged to main; runtime code, examples, templates, and scaffolder untouched — the migrated Todo and generated starter were the SOURCE material, and the verification battery proves the docs did not drift from them.
- README: the facade section replaces the stale runFormAction() description — defineFormAction() + createFormActions() presented as the preferred application-level workflow, with runFormAction() named as the lower-level compatibility surface.
- getting-started §4 rewritten around the facade; the embedded snippet byte-matches its rewritten runnable module (docs/snippets/guides/getting-started-form.ts — CI-executed: invalid 422 in both worlds, valid PRG 303 via docs/snippets/forms.ts). No legacy call remains anywhere in the getting-started path.
- New canonical guide docs/guides/form-actions.md: mental model (validated input → run() → Result → success.fragment() → redirect/fragment), the run/Result/fragment invariant with the explicitly rejected mutation-inside-fragment anti-pattern, dialect binding once, handle() as default vs execute() for the discriminated outcome, invalid fragments/documents/target, field helpers, transactions, cancellation, and the low-level runFormAction() section.
- New docs/guides/form-action-migration.md: legacy before-example, facade after-example, the full mechanical mapping table (action.fragment → run + success.fragment; renderForm → invalid.fragment; renderInvalidDocument → invalid.document; formTarget → invalid.target; outcome.response → forms.handle(); errors.first → field(name).error; String(submitted) → field(name).value/.values; per-route { dialect } → bound once), and the compatibility statement: runFormAction() remains supported during the documented pre-1.0 compatibility window; no invented removal dates.
- New docs/guides/validation.md: the InvalidFieldView shape, the multi-value behavior table (missing/scalar/duplicates), the guarantees (value is first scalar; values preserve order; never comma-joined; error is field-first; globals never leak; redaction owned upstream), and the String(render.submitted[...]) anti-pattern with the field(name).value ?? "" replacement.
- Todo walkthrough documents mutation/result/render separation with the actual create run() (item + counts), the discriminated RenameTodoResult rationale, and the intentional toggle/delete boundary. Architecture guide gains the package-boundary section: @bundar/forms owns neutral workflow semantics; @bundar/htmx owns HTTP/HTMX delivery; defineFormAction() is an inference helper, createFormActions() binds delivery once, and the facade is an additive adapter — not a controller or runtime layer.
- API references regenerated via docs:generate — forms runtime exports 11 → 13 (executeExecutableFormAction, invalidField now listed); htmx page carries the facade exports. Only generated files were touched through the generator.
- Claims/drift guard added (tests/docs/form-action-docs.test.ts, 8 tests): preferred-API presence in README/getting-started/canonical guide; old-smell absence (runFormAction call, String(render.submitted, errors.first, .then((outcome)=>outcome.response)) in preferred-path snippets; the mapping table and compatibility wording; the package-boundary claims; the multi-value documentation. runFormAction is deliberately NOT banned globally — it must remain in the compatibility section, the migration before-example, and low-level docs.
- Gates: docs:check, docs:snippets, tests/docs 27/0 (incl. the byte-match guide test), typecheck, lint, format:check, build — all exit 0; full suite 1,311 pass / 0 fail across 165 files; drift checks against real code: test:example (todo/admin/no-js), test:template minimal-htmx2, test:scaffold all green. Candidate regeneration 9-step exit 0 (manifest rebound to cd3270c); release:verify 42/42; registry preflight 9/9. #191 untouched.
- Authoritative battery 33233248367 on main ca877a9: success on the FIRST attempt; bundle `release-candidate-artifacts-ca877a9…`, GitHub artifact digest sha256:0429335c7b4c7d63dc55bb2aa1bd6242292de3da188bdc687ca18b32203d7a8b. Model B rehearsal from the downloaded bundle green: dry-run exit 0 (NOTHING published) and registry preflight 9/9 on-disk SHA-256 matches. Epic complete: all eight children (#180–#187) merged; legacy reference (admin CRUD) still on runFormAction and passing; no legacy export removed; canary candidate untouched.

## 2026-08-29 — GH-191 resolution: reference-environment benchmark policy + docs link guard

- Docs-only fix first (5d542ff): the canonical form-actions guide's two relative links (guides/validation.md, guides/form-action-migration.md written from inside docs/guides/) corrected, plus three latent breaks the new guard caught immediately (repo-root-relative starter link in getting-started, a docs/-prefixed link, an api/README pointer to a nonexistent guides/README.md). scripts/docs-check.ts now walks README + docs/** and fails on any relative .md link that does not resolve against the linking document.
- #191 policy implemented (74ff8d0): benchmark budget enforcement is bound to the declared REFERENCE environment — the public Candidate Release Battery, which opts in via BUNDAR_BENCH_REFERENCE=1 (set in candidate-release.yml with a named fingerprint). Everywhere else the same comparison runs ADVISORY: it prints the environment fingerprint (bun/platform/threads + the raw-Bun startup baseline as the drift canary) and labels timing breaches ADVISORY without failing the run; parity, the BR-004 semantic guard, and missing budgets still fail closed EVERYWHERE (correctness is never advisory). --generate is reference-only — pooling a drifted machine would enshrine the drift as the baseline. Local verification: advisory exit 0 with the 49.7ms startup breach reported as advisory; reference exit 1. The battery environment passes its budgets, so enforcement there is unaffected.
- Policy tests (tests/benchmark/regression-policy.test.ts): mode selection (env/flag/other-value), boundary evaluation (clean/alert/fail), missing-budget fail-closed, staleness alerts, and the startup drift canary. regression.ts gained exported pure pieces (isReferenceEnvironment, evaluateBudgets, BenchArtifact) with the script body under import.meta.main.
- Policy documented in docs/performance/alpha.md (§ Reference-environment policy): reference = public battery (GitHub-hosted ubuntu-latest, Bun 1.4.0, pooled 3-run budget generation, reviewed commit of alpha-budgets.json); everywhere else advisory measurements with fingerprint + baseline, no local regeneration.
- Gates: full suite 1,316 pass / 0 fail across 166 files; typecheck, lint, format:check, docs:check, docs:snippets exit 0; candidate regeneration rebound to 74ff8d0; release:verify 42/42; registry preflight 9/9.
- Authoritative battery 33237699431 on main daa20d4: success WITH BUNDAR_BENCH_REFERENCE=1 enforced — the reference environment passes its budgets under the declared policy. Bundle `release-candidate-artifacts-daa20d4…`, digest sha256:f9c4427bc21644f6823f3454a0faf0d9b2da226816fb19d8cd0776423ea8dc46. Model B rehearsal from the downloaded bundle green (dry-run exit 0, NOTHING published; registry preflight 9/9).

## 2026-08-29 — GH-176: approved 1.0 support matrix (Option A) landed

- Maintainer decision recorded in the GH-176 thread: Option A (conservative) with clarified evidence terminology. The agent phase produced only the evidence inventory + options; the approval comment is the authority for every claim below.
- Decision landed (46cce4b): new docs/compatibility/support-matrix.md as the canonical matrix — Bun-only runtime; installation minimum (Bun >=1.4.0, engines + fail-closed preflight) stated as DISTINCT from the release-verified reference (the exact Bun pinned by the Candidate Release Battery, currently 1.4.0; newer releases adopt via an advisory lane and the docs must never imply blanket conformance); Linux x64 production; macOS scoped to the exact approved sentence (local development may work, not part of the verified production support matrix, best-effort, not release-blocking); Windows/arm64 not claimed; browser conformance verified against the Chromium revision supplied by the pinned Playwright toolchain, Firefox/WebKit explicitly out of scope; htmx supported line 2.0.x with 2.0.10 as tested reference and a battery-gated upgrade policy; htmx 4 experimental/opt-in/non-default; maintenance shape with every duration delegated to GH-178 and the current no-LTS/no-backport/no-SLA truth retained until then.
- Machine-readable narrowing: the v2 dialect's declared supportedRange is now >=2.0.0 <2.1.0 (the tested 2.0.x line) instead of the evidence-free >=2.0.0 <3.0.0; contract + profile tests updated. The htmx upgrade policy is documented in the matrix: a new htmx patch becomes the tested reference only after the full dialect + browser battery passes.
- browsers.md Chromium claim tightened to the approved sentence (verified against the revision bundled with the pinned Playwright toolchain). README status table + Development section, SUPPORT.md scope, and SECURITY.md supported-scope statement aligned to the matrix. No new CI lanes — Option A requires none by design.
- Drift guard: tests/compatibility/support-matrix.test.ts pins the exact approved wording constraints (macOS sentence, Chromium sentence, minimum-vs-reference distinction, GH-178 delegation, no-LTS statement), the narrowed machine-readable range, the engines metadata, and the absence of the old wider range and broader browser claim.
- Gates: htmx suite 241 pass / 0 fail; full suite 1,321 pass / 0 fail across 167 files; typecheck, lint, format:check, docs:check exit 0; candidate regeneration rebound to d1ef0fb; release:verify 42/42; registry preflight 9/9. #191 policy untouched. Next in dependency order: GH-178 (policy documents — durations, channels, EOL) then GH-177/GH-179.
- Authoritative battery 33241520190 on main 7b072ef: success; bundle `release-candidate-artifacts-7b072ef…`, digest sha256:d1b8acda316a6d5e55e762ebd65cc6a35d10c2932258ce388e0941f57416e09c. Model B rehearsal from the downloaded bundle green (dry-run exit 0, NOTHING published; registry preflight 9/9). GH-176 complete per its contract: the decided matrix landed in docs/compatibility/ and is reflected by README and the support documents, with drift tests pinning the approved claims.

## 2026-08-29 — GH-178 stage 1: approved support/security policy landed (two-stage; #178 stays open on publishing)

- Stage 1 (4822345): the maintainer's Option A policy landed in SECURITY.md and SUPPORT.md, resolving the flagged contradiction — the 7-day promise is retained and NAMED an acknowledgement target (not a resolution SLA), joined by a maintainer-approved 14-calendar-day initial assessment target (severity + affected versions + fix-or-wontfix decision, when reproduction details suffice) and an explicit no-remediation-SLA statement.
- SECURITY.md restructured: event-based supported versions (before 1.0: main + newest published prerelease; after 1.0: newest stable minor in the current major; EOL on supersession, never on a calendar), supported channel table (canary/alpha/beta/rc newest-only fix-forward, rc blocker-only, latest stable-only per ADR-0021), response targets, coordinated disclosure (embargo by severity/exploitability/release readiness, credits, safe-harbor), backport policy (newest stable minor only; older-minor critical backports maintainer-discretionary, not guaranteed; LTS none), deprecation operational rule (one stable minor of documented notice in docs + declarations; breaking removals only in a major after 1.0; security exception), and EOL communication rules (release notes state channel, superseded version, EOL status, migration notes, residual risks; EOL versions stay installable, best-effort fixes create no promise). The GH-176 platform/runtime scope is preserved verbatim-linked.
- SUPPORT.md: channel table, version-support table, how-to-get-help routing table (private vulnerability reporting / issues / discussions-when-enabled / no private-commercial support), backports, deprecation, no-LTS statement, EOL rules; compatibility pointer realigned to docs/compatibility/support-matrix.md; the exact GH-176 macOS/platform sentences preserved.
- publishing.md: still status: draft, now with an explicit provisional status box — operationally tested in rehearsal, final only after GH-130/GH-132 complete the first live canary publication with registry verification.
- Drift guard: tests/docs/policy-claims.test.ts (9 tests) pins the approved targets verbatim, channel table, event-based versions, no-LTS as explicit denial only (every LTS occurrence must sit inside a denial phrase), no-guaranteed-old-minor-backport, deprecation rule, support-matrix reference consistency, and negative assertions (no all-Bun/cross-platform/all-modern-browsers/Windows-supported/arm64-supported/24-7/guaranteed-response broadening in canonical policy sections). The support-matrix drift test's SECURITY assertion realigned to the landed decision ("approved in GH-178").
- Two-stage closure: #178 remains OPEN — stage 2 (publishing guide finalization from actual live-publish evidence, rollback/revocation verification) waits on GH-130/GH-132 completing the first real canary publication.
- Gates: docs suites + policy claims 9/0, docs:check, typecheck, lint, format:check, build exit 0; full suite 1,330 pass / 0 fail across 168 files; candidate regeneration rebound to 668d0c0; release:verify 42/42; registry preflight 9/9. #191 policy untouched.
- Authoritative battery 33243707617 on main c6997d4: success; bundle `release-candidate-artifacts-c6997d4…`, digest sha256:10b07597b41591b0c4a577acad88456702f1138de23b18c464f8c7ea79fbc6e3. Model B rehearsal from the downloaded bundle green (dry-run exit 0, NOTHING published; registry preflight 9/9). GH-178 remains OPEN by design: stage 2 (publishing-guide finalization from live-publish evidence + rollback/revocation verification) waits on GH-130/GH-132 completing the first real canary publication.

## 2026-08-29 — GH-178 stage-1 follow-up: support-matrix drift correction + Docs-site workflow un-red

- Consistency patch (9897cb1 → merge 0218c7c): the support matrix's Maintenance section still deferred durations to GH-178 ("Delegated to GH-178", "Until that decision lands") after the stage-1 policy landed — replaced with the approved event-based policy summary pointing at SECURITY.md/SUPPORT.md. Drift guards extended in both directions: the compat matrix test now REQUIRES the landed wording; the GH-178 claims test gains a negative test rejecting both stale delegation phrases (load-bearing against the pre-fix text). Tracker realigned: #178 labels moved from status:needs-decision to status:blocked + blocked-by-#130 + blocked-by-#132; issue stays OPEN on the unchanged stage-2 checklist.
- Pre-existing red triaged on the same push: the Docs site workflow had been failing on EVERY main push since the documentation wave (last green at the website merge b655dfd) — unnoticed because docs:site:check was not in the per-wave local gate list. Root cause: three source docs synced into the developer path carried engineering-record markers (examples/todo.md "GH-185" heading, guides/architecture.md "GH-180 series" heading, compatibility/browsers.md "GH-176" sentence), which docs:site:check bans outside release-notes pages. Fixed at the sources (2adaaab → merge 7ee8124, 3 lines), site re-synced (33 pages) and rebuilt (35 pages) green. LESSON (process): any wave touching docs/** must run docs:site:sync + docs:site:check (+ the website build when website deps are installed) locally, not rely on CI.
- Full suite 1,331 pass / 0 fail across 168 files; typecheck, lint, format:check, docs:check, docs:site:check exit 0; candidate regeneration 9/9; release:verify 42/42; registry preflight 9/9. #191 policy untouched.
- Authoritative battery 33246132876 on main 96bc93e: success; bundle `release-candidate-artifacts-96bc93e2742fd9c7a19e9c7dbd118cc0d74635ed`, GitHub artifact digest sha256:cfb0160fcecb610ebe4efeaa65bc6d301c7ab831339f218ae3b4a531afaa8092. Model B rehearsal from the downloaded bundle green: dry-run exit 0 (NOTHING published); preflight 9/9.
- Operator lesson (registry preflight from a bundle): --manifest and --root-dir must be passed TOGETHER. Omitting --manifest silently loads the repo-local committed manifest and hashes bundle bytes against it → spurious "[disk] SHA-256 mismatch" noise. Verified after correction: the bundle is internally consistent (candidate-identity.json == candidate-manifest.json == all nine on-disk tarball SHA-256 values). Corroborating observation: tgz BYTES are not cross-machine deterministic (cli tarball packed locally = 15d56369…, CI-packed in the bundle = a06596d8…) while release:reproduce's contract (unpacked trees byte-identical) holds — Model B's binding of identity to the bundle's own re-pack is exactly what makes the candidate authoritative.

## 2026-08-29 — GH-177 preparation: independent-review packet landed (review itself awaits an independent reviewer)

- Packet (security-review/, 8 documents, 01ece76 → merge a8cafd8): scope.md (GH-177 areas expanded into concrete sub-surfaces, out-of-scope list, P0–P3 definitions); architecture-map.md (security-load-bearing files across all eight packages + audits + release chain, each with its role); trust-boundaries.md (nine boundaries, each with enforcement points and reviewer questions); threat-model.md (five adversaries; promised vs explicitly-not-promised defenses; impact hierarchy); test-inventory.md (coverage map + audit-vs-test asymmetry heuristic); known-residual-risks.md (documented limitations ONLY — the reviewer verifies each and treats the list as a floor); reviewer-instructions.md (independence requirement as a hard rule — implementation agents and AI assistants that authored/directed the code do not qualify; environment; method; private-disclosure routing per SECURITY.md; no gate-weakening); finding-template.md (required-field finding record + coverage-summary header). Every referenced file path machine-verified to resolve; no findings/verdicts/acceptances recorded — that judgment belongs to the independent reviewer per the issue contract.
- #177 stays OPEN on status:waiting-external-participant; the packet is preparation, not performance. Findings/dispositions will be recorded by the reviewer in security-review/findings.md and become the M9 gate evidence.
- Gates: full suite 1,331 pass / 0 fail across 168 files; typecheck, lint, format:check, docs:check exit 0; candidate regeneration 9/9 (manifest rebound to a8cafd8); release:verify 42/42; registry preflight 9/9.
- Authoritative battery 33247265939 on main 693a918: success; bundle `release-candidate-artifacts-693a918…`, GitHub artifact digest sha256:1727b003736d7ed6f7ef85b3b694a1d6ae89930b0a70485b9a0859311e04f2bf. Model B rehearsal from the downloaded bundle green: `publish:approved --dry-run` exit 0 (NOTHING published) and `registry:verify --preflight` 9/9 with --manifest + --root-dir both pointed at the bundle (the flag pairing is now the recorded invocation).

## 2026-08-29 — Maintainer hold: handoff status page landed; agent-executable work frozen

- Per the maintainer's hold decision, all feature/API surfaces are frozen until the canary or the independent review finds real need (route API, middleware API, form-action facade, session contract, JSX contract, testing facade, package exports, CLI surface, typed URLs, candidate format). Allowed before canary: review/battery/publication blockers, factual docs corrections, proven flaky/infra fixes, urgent security response.
- Handoff landing page (docs/maintainers/handoff.md, 1361dae → merge 24cad93b, release commit 5c7e4fc): current-state table (12 open issues grouped — external participant #177/#135, human gate #130, chain #132/#133/#134/#136, M9 sequenced #173/#174/#175, terminal #179; 0 agent-executable), waiting-on list, freeze + allowed + do-not lists, the exact two-flag bundle preflight command with the --manifest operator warning, the #177 reviewer handoff package list, and the post-hold human sequence (#130 checklist, #132 flow, #178 stage 2, beta chain with independent #135 participants, #179 preconditions). Not synced to the public site (maintainers/ excluded by design); docs:check + docs:site:check verify it.
- Full suite 1,331 pass / 0 fail across 168 files; typecheck/lint/format/docs gates exit 0; candidate regeneration 9/9 (manifest rebound to 24cad93b); release:verify 42/42; registry preflight 9/9.
- Authoritative battery 33248138029 on main 5c7e4fc: success; bundle `release-candidate-artifacts-5c7e4fc…`, GitHub artifact digest sha256:8aed0f25a2b88a1c63e7420775d4e8e94b2b8fc696a8a88f69d848c7a7205388. Model B rehearsal from the downloaded bundle green (dry-run exit 0, NOTHING published; preflight 9/9 with both flags).
- HOLD STATE: repository intentionally quiet. Remaining work is human/external only: independent reviewer executes #177 from the packet; maintainer completes #130; #132 live canary; #178 stage 2; beta chain; #179 terminal. No new issues, waves, or framework changes until external input arrives.
